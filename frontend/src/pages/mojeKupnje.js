...
const MojeKupnje=()=>{
    ...
    useEffect(() => {
        ...
        if (logiraniKorisnikJSON) {
            ...
            usersActions.dohvatiJednogKorisnika(korisnik.username)
                .then(res => setUser(res.data))
            kupnjeAkcije.dohvatiSve()
                .then(res => postaviKupnje(res.data.filter(k=> k.korisnik.username===korisnik.username)))
    ...
    return(
        <div>
            ...
            <thead>
                ...
                <tbody>
                    {kupnje.map(k=>
                        <Kupnje key={k.id} car={k.car} tire={k.tire} rim={k.rim} kupac={user.ime}/>)}
                </tbody>
            </table>
        </div>
    )
