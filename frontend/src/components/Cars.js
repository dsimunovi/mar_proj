...
const Cars = ({
  ...
  const pronadiNaplatak = () => {
    if (!checkNaplatak) {
      rimsActions.dohvatiJedanNaplatak(chosenRim).then((res) => {
        setRims(res.data);
      });
    } else {
  ...
  const pronadiGumu = () => {
    if (!checkGuma) {
      tiresActions.dohvatiJedneGume(chosenTire).then((res) => {
        setTires(res.data);
      });
    } else {
        ...
        <Modal.Body>
          ...
          <div className="flex-row-1">
            ...
            <div className="flex-column-2">
              ...
              {visibleGume ? (
                <div>
                  <img className="slike-sel" src={tires.slika} />
                  <div>Marka: {tires.marka}</div>
                  <div>Tip: {tires.tip}</div>
                  <div>Cijena u €: {tires.cijena}</div>
                </div>
              ) : (
                ""
              )}
            ...
            <div className="flex-column-2">
              ...
              {visibleRim ? (
                <div>
                  <img className="slike-sel" src={rims.slika} />
                  <div>Tip: {rims.tip}</div>
                  <div>Cijena u €: {rims.cijena}</div>
                </div>
              ) : (
                ""
              )}
          ...
          {/*flex-row-1*/}
          <div id="botun">
            <button className="postavi" onClick={changeColor}>
              Postavi boju
            </button>
            <button className="postavi" onClick={pronadiGumu}>
              Postavi gume
            </button>
            <button className="postavi" onClick={pronadiNaplatak}>
              Postavi naplatke
            </button>
          </div>
        </Modal.Body>
                  ...
                  <div className="card">
                            ...
                            <div className="input-group">
                              <input
                                ...
                                value={brKartice
                                  ...
                                  .replace(/(.{4})/g, "$1 ")
                                  .trim()}
                                onChange={(e) => postaviKarticu(e.target.value)}
                              />
                      ...
                      <div className="row">
                        <div className="form-group col-sm-4">
                          ...
                          <select
                            className="form-control"
                            id="ccmonth"
                            onChange={(e) => postaviIstekMjesec(e.target.value)}
                          >
                            <option>1</option>
                            <option>2</option>
                            <option>3</option>
                        ...
                        <div className="form-group col-sm-4">
                          ...
                          <select
                            className="form-control"
                            id="ccyear"
                            onChange={(e) => postaviIstekGodina(e.target.value)}
                          >
                            <option>2023</option>
                            <option>2024</option>
                            <option>2025</option>
                    ...
                    <div className="card-footer">
                      <button
                        ...
                        onClick={kupnjaAuta}
                      >
                        <i className="mdi mdi-gamepad-circle"></i> Kupi
                      </button>
                      <button className="btn btn-sm btn-danger" type="reset">
                        <i className="mdi mdi-lock-reset"></i> Odustani
                      </button>
                    </div>
                  </div>
                </div>
