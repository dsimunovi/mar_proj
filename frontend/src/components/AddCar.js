...
const AddCar=()=>{
    ...
    const dodajAuto=(e)=>{
        e.preventDefault()
        try{
            if(slika===''||marka===''||model===''||snagaMotora===''||godiste===''|| kilometri===''||cijena===''||vrstaMotora===''||
            mjenjac===''||gumaAuta==='' ||naplatakAuta==='' || vrstaMotora==="Odaberi vrstu motora" || mjenjac==="Odaberi vrstu mjenjača"){
                alert("Niste popunili sve podatke!")
                return;
            }
        const novoAuto={
...
<label>URL slike auta:<input onChange={(e)=>postaviSliku(e.target.value)}></input></label>
<label>Marka auta: <input onChange={(e)=>postaviMarku(e.target.value)}></input></label>
<label>Model auta:<input onChange={(e)=>postaviModel(e.target.value)}></input></label>
<label>Snaga motora u kW:<input onChange={(e)=>postaviSnaguMotora(e.target.value)}></input></label>
<label>Godište auta: <input onChange={(e)=>postaviGodinu(e.target.value)}></input></label>
<label>Prijeđena kilometraža auta: <input onChange={(e)=>postaviKilometre(e.target.value)}></input></label>
<label>Cijena auta: <input onChange={(e)=>postaviCijenu(e.target.value)}></input></label>
<label>Gume auta: <span>{gumaAuta}</span></label>
<label>Naplatci auta: <span>{naplatakAuta}</span></label>
            </div>
            <div className="stupac">
                <h3>Gume na autu</h3>
      ...
      />
                <select onChange={(e)=>postaviVrstuMotra(e.target.value)}>
                    <option>Odaberi vrstu motora</option>
                <option>Benzin</option>
                <option>Dizel</option>
            </select>
            <select onChange={(e)=>postaviMjenjac(e.target.value)}>
                <option>Odaberi vrstu mjenjača</option>
                <option>Mehanički</option>
                <option>Automatski</option>
            </select>
            <button id="dodaj" onClick={dodajAuto}>Dodaj auto</button>
         </div>
